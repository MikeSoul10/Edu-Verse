  ...
  <section>
    ...
    {apuntes.length === 0 ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {apuntes.slice(0, 3).map((apunte) => ( 
          <div key={apunte.apunte_id} className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all flex flex-col">
            {/* Hacemos que la parte superior lleve al Detalle */}
            <Link to={`/apunte/${apunte.apunte_id}`} className="group">
              <div className="w-full h-24 bg-blue-100 rounded-xl mb-4 flex items-center justify-center text-3xl group-hover:bg-blue-200 transition-colors">📄</div>
            ...
            <div className="mt-4 pt-4 border-t flex justify-between items-center">
              ...
              <div className="flex gap-2">
                ...
                <Link 
                  ...
                  className="text-[10px] bg-blue-50 text-blue-600 px-2 py-2 rounded-lg font-bold hover:bg-blue-100"
                >
                  ⭐ Valorar
                </Link>
                {/* Botón directo al PDF */}
                <a 
                  href={`http://localhost:4000${apunte.archivo_url}`} 
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="text-[10px] bg-gray-800 text-white px-2 py-2 rounded-lg font-bold hover:bg-black"
                >
                  Ver PDF
                </a>
              </div>
